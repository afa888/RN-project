...
let pageSize = 1;
let total = 0;
export default class SeachGameGridListScreen extends Component<Props> {
    ...
    forwardGame = (item) => {
        ...
        http.post('game/forwardGame', prams).then((res) => {
            if (res.status === 10000) {
                console.log(res)
    ...
    renderFooter = () => {
        ...
        if (this.state.data.length != 0 && this.state.isLoreMoreing == 'LoreMoreing') {
            return (
                <View style={{
                    height: 44,
